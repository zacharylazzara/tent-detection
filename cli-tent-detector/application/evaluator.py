...
def evaluator(prediction_output_dir, data: pd.DataFrame, operators: list[Operator], transformations: transforms.Compose = None, **kwargs) -> dict[str, dict[str, str | None]]:
    ...
    for operator in operators:
        if kwargs.get('t_loader') and kwargs.get('v_loader'):
            operator.train(kwargs.get('epochs', 1), kwargs.get('t_loader'), kwargs.get('v_loader'), tqdm,
                        save_model=True, save_history=True)
        p_data = operator.predict(loader, tqdm, save_predictions=True)[1].combine_first(data).sort_index()
        paths[str(operator)] = save_data(prediction_output_dir, p_data, transformations, tqdm, y='p', heatmap_title='Detected Tents per Region', **kwargs)
